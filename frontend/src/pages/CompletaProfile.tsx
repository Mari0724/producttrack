...
import { CheckCircle, User, Building2, Mail, Phone, MapPin, Lock } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-yellow-800">¡Completa tu perfil!</h1>
      ...
      <div className="mb-6 p-4 border-l-4 border-yellow-500 bg-white shadow rounded">
        ...
        <p className="text-sm text-gray-600">Esta información fue proporcionada por tu empresa y no se puede modificar.</p>
        <ul className="mt-3 space-y-1 text-sm">
          <li className="flex items-center"><Mail className="h-4 w-4 text-gray-500 mr-2" /> <strong>Correo:</strong> {preRegisteredData.email}</li>
          <li className="flex items-center"><Building2 className="h-4 w-4 text-gray-500 mr-2" /> <strong>Empresa:</strong> {preRegisteredData.companyName}</li>
          <li className="flex items-center"><User className="h-4 w-4 text-gray-500 mr-2" /> <strong>Rol:</strong> {preRegisteredData.role}</li>
        </ul>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded p-6 space-y-6">
        <div>
          ...
          <input id="fullName" type="text" value={formData.fullName} onChange={e => handleInputChange('fullName', e.target.value)}
        ...
        <div className="border-t pt-4">
          ...
          {formData.newPassword && (
            <div>
              <label className="block text-sm font-medium mb-1" htmlFor="confirmPassword">Confirmar nueva contraseña</label>
        ...
        <button type="submit" disabled={isSubmitting} className="w-full bg-yellow-700 hover:bg-yellow-800 text-white py-3 rounded">
      ...
      <div className="mt-6 p-4 bg-yellow-100 rounded">
        <p className="text-sm text-gray-700">
          ...
          Una vez completes tu perfil, podrás acceder a todas las funcionalidades de ProductTrack según tu rol asignado.
        </p>
      </div>
    </div>
  );
